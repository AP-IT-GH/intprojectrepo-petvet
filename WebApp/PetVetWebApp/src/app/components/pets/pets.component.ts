...
export class PetsComponent implements OnInit {
    ...
    this.data.getPetData(this.ownerId).subscribe(
      (pet) => {
        ...
        this.pets.forEach((x, index) => {
          ...
          this.data.getFullPetData(x.petId).subscribe(
            (petdata) => {
  ...
  put(pet, index) {
    if (this.chosenVet == "") {
      ...
      this.data.PostPetVet(this.chosenVet, pet.petId, pet.name).subscribe(
        (pet) => {
          this.postvet = [];
          this.postvet = pet;
  ...
  onEnter() {
    ...
    else window.alert("No proper Name");
    this.petnameInput = "";
  }
  ...
  PostPet(vetId, petName, petId, index) {
    ...
    this.data.PostPetVet(vetId, petId, petName).subscribe(
      (postpet) => {},
      (error) => {
        console.log(error);
      }
...
export class DialogDel {
  ...
  onYesClick(id): void {
    if (
      this.data.petNameret == undefined &&
      this.data.vetId == undefined &&
      id == 2
    )
